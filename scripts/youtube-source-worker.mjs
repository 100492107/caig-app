...
async function process(job) {
  ...
  try {
    ...
    const { error } = await supabase.from('local_ai_jobs').update({
      ...
      production_status: 'youtube_downloaded_waiting_analysis',
      error_message: null,
    }).eq('id', job.id);
    if (error) throw error;
    ...
    await supabase.from('local_ai_jobs').update({
      ...
      error_message: message.includes('No module named') || message.includes('yt_dlp')
        ? 'yt-dlp is not installed in the local YouTube environment. Run the Cornerstone YouTube setup command, then retry.'
        : message,
