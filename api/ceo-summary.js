...
function nextAction(data) {
  if (data.samples_due > 0) return { label: 'Produce the oldest dealer sample', detail: `${data.samples_due} sample${data.samples_due === 1 ? '' : 's'} due`, href: '/outreach' };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [samples, replies, calls, pilots, creative] = await Promise.all([
