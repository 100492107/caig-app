...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      if (['outreach_status', 'job_status'].includes(action)) { if (!id || !/^[0-9a-f-]{36}$/i.test(id)) return res.status(400).json({ error: 'Valid job id required' }); const job = await readJob(SERVICE_KEY, id, ownerId); if (!job) return res.status(404).json({ error: 'Job not found' }); return res.status(200).json(job); }
      if (action === 'generations') { const page = await listGenerations(SERVICE_KEY, ownerId, { limit: req.query?.limit ?? 100, offset: req.query?.offset ?? 0 }); return res.status(200).json({ jobs: page.rows, total: page.total, offset: page.offset, limit: page.limit, nextOffset: page.nextOffset, hasMore: page.hasMore }); }
