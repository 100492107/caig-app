// Shared serverless bridge for Revenue Recovery, Content Engine and Track B source ingestion.
...
function normaliseBusiness(business = {}) {
  return {
    id: clean(business.id),
    name: clean(business.name || business.businessName || business.dealerName),
    decisionMaker: clean(business.decisionMaker || business.ownerName || business.contact),
    email: clean(business.email), website: clean(business.website),
    location: clean(business.location || business.city || business.postcode),
    vertical: clean(business.vertical || business.industry),
...
function buildOutreachPrompt(business) {
  return `You are the senior outbound strategist for Cornerstone AI Enterprise's Track A Revenue Recovery operation.\n\nWORKSPACE / RESEARCH DOMAIN — HARD LOCK:\nTRACK_A_REVENUE_RECOVERY only. The niche is the problem domain: revenue leakage created when enquiries, leads, conversations, appointments, quotes or opportunities go cold, stale, unworked or fail to move toward a decision. Target businesses can be any legitimate lead-driven business.\n\nBUSINESS DATA — USE ONLY WHAT IS PROVIDED. NEVER INVENT FACTS.\n${JSON.stringify(business, null, 2)}\n\nCOMMERCIAL PATH:\nTarget account -> Recovery conversation -> Leakage diagnosis -> Controlled test -> Measured recovered opportunities -> Repeat / recurring support.\n\nEMAIL KPI: Get a reply from the right decision-maker. Start with the prospect's world. Surface a plausible leakage point. Ask one simple diagnostic question. Do not use pricing, feature dumps, AI jargon, fake certainty or unsupported claims. Do not pretend an unverified leakage point was audited.\n\nRETURN JSON ONLY:\n{\"recommended_subject\":\"\",\"subject_options\":[\"\",\"\",\"\"],\"email\":\"\",\"why_this_should_get_a_reply\":\"\",\"pattern_interrupt\":\"\",\"recovery_opportunity\":\"\",\"discovery_questions\":[\"\",\"\",\"\"],\"cta\":\"\",\"followup_plan\":{\"day_2_angle\":\"\",\"day_3_angle\":\"\",\"day_4_angle\":\"\",\"day_5_angle\":\"\"},\"quality_gate\":{\"prospect_first\":\"PASS\",\"problem_fit\":\"PASS\",\"replyability\":\"PASS\",\"specificity\":\"PASS\",\"human_voice\":\"PASS\",\"no_unsupported_claims\":\"PASS\",\"research_domain\":\"TRACK_A_REVENUE_RECOVERY\"}}`;
}
...
async function readJob(serviceKey, id) {
  ...
  const text = await r.text(); if (!r.ok) throw new Error(`Supabase read failed: ${text}`);
...
async function listGenerations(serviceKey, { limit = 1000, offset = 0 } = {}) {
  ...
  const rows = JSON.parse(text).map(sanitiseJob); const range = r.headers.get('content-range') || ''; const m = range.match(/\/([0-9]+)$/); const total = m ? Number(m[1]) : null;
...
async function deleteGeneration(serviceKey, id) {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/local_ai_jobs?id=eq.${encodeURIComponent(id)}`, { method: 'DELETE', headers: { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` } });
  const text = await r.text(); if (!r.ok) throw new Error(`Supabase delete failed: ${text}`);
}
async function createJob(serviceKey, row) {
  ...
  const text = await r.text(); if (!r.ok) throw new Error(`Supabase queue failed: ${text}`);
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      if (['outreach_status', 'job_status'].includes(action)) {
        if (!id || !/^[0-9a-f-]{36}$/i.test(id)) return res.status(400).json({ error: 'Valid job id required' });
        const job = await readJob(SERVICE_KEY, id); if (!job) return res.status(404).json({ error: 'Job not found' });
        return res.status(200).json(job);
      }
      if (action === 'generations') {
        const page = await listGenerations(SERVICE_KEY, { limit: req.query?.limit ?? 1000, offset: req.query?.offset ?? 0 });
  ...
  if (body.action === 'delete_generation') {
    ...
    try { await deleteGeneration(SERVICE_KEY, id); return res.status(200).json({ success: true }); }
  ...
  if (body.action === 'queue_media_ingestion') {
    ...
    const fileName = clean(body.fileName) || 'source';
    ...
    if (!objectPath.startsWith(`${clean(body.userId)}/`) || !clean(body.userId)) return res.status(400).json({ error: 'objectPath must be scoped to the authenticated user' });
    try {
      const created = await createJob(SERVICE_KEY, {
        title: `Track B source ingestion · ${fileName}`,
        job_type: 'content_media_ingestion',
        model: OUTREACH_MODEL,
        persona_id: 'cornerstone_content_engine',
        system_prompt: 'You are the Track B media ingestion controller. Download the private source, extract evidence, then hand off to Qwen text and vision analysis.',
        user_prompt: `Ingest source media from ${SOURCE_BUCKET}/${objectPath}.`,
        options: { bucket: SOURCE_BUCKET, object_path: objectPath, file_name: fileName, content_type: contentType, research_domain: 'TRACK_B_CONTENT_ENGINE', workspace_id: 'track_b' },
        status: 'queued', production_status: 'source_queued',
      });
  ...
  if (body.action === 'queue_outreach') {
    const business = normaliseBusiness(body.business || body.dealer);
    if (!business.name) return res.status(400).json({ error: 'business.name required' });
    try {
      const created = await createJob(SERVICE_KEY, { title: `Track A Revenue Recovery · ${business.name} · Email ${business.emailStage + 1}`, job_type: 'trend_scan', model: OUTREACH_MODEL, persona_id: 'cornerstone_track_a_revenue_recovery', system_prompt: 'You are Cornerstone AI Enterprise\'s Revenue Recovery strategist. Prospect-first. Problem-led. Evidence-grounded. This is not a dealership-only workflow.', user_prompt: buildOutreachPrompt(business), options: { max_tokens: 6500, temperature: .52, research: true, outreach: true, recovery_business_id: business.id, research_domain: 'TRACK_A_REVENUE_RECOVERY', workspace_id: 'track_a', research_firewall: true }, status: 'queued', production_status: 'not_started' });
  ...
  if (body.action === 'queue_content_engine') {
    try {
      const payload = { niche: body.niche, channel: body.channel, referenceUrl: body.referenceUrl, referenceNotes: body.referenceNotes, duration: body.duration, output: body.output, direction: body.direction, sourceAnalysis: body.sourceAnalysis };
      const created = await createJob(SERVICE_KEY, {
        ...
        options: { max_tokens: 16000, temperature: .55, research: true, content_engine: true, research_domain: 'TRACK_B_CONTENT_ENGINE', workspace_id: 'track_b', reference_url: clean(payload.referenceUrl), niche: clean(payload.niche), has_source_analysis: Boolean(payload.sourceAnalysis) },
