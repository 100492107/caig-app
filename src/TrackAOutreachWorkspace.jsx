    ...
    <div className="ta-outreach-inner">
      <div className="ta-outreach-head">
        <div>
          ...
          <p style={{ margin: 0, color: '#98a2b1', maxWidth: 800, lineHeight: 1.65 }}>Qwen researches the revenue-leakage angle and writes a prospect-first message. The aim is to identify money already entering the business but disappearing before the deal closes.</p>
        </div>
      ...
      {!result ? <section style={{ minHeight:280, display:'grid',placeItems:'center',...card }}><div style={{ textAlign:'center',padding:'12px' }}><div style={{ fontSize:34, marginBottom:10 }}>↗</div><div style={{ fontSize:18,fontWeight:900 }}>Building the recovery outreach package</div><div style={{ color:'#7f899a',marginTop:7 }}>{status}</div>{jobId && <div style={{ color:'#586273',fontSize:10,marginTop:10 }}>Job {jobId.slice(0,8)}</div>}</div></section> : <div style={{ display:'grid',gap:14 }}>
        ...
        <section style={card}><div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap' }}><div style={{ ...label }}>Recovery email</div><button onClick={()=>copy(result.email,'body')} style={{ background:'#0d1118',color:'#cfd5df',border:'1px solid #2a3340',borderRadius:10,padding:'10px 12px',cursor:'pointer',minHeight:44 }}>{copied==='body'?'Copied':'Copy email'}</button></div><pre style={{ whiteSpace:'pre-wrap',fontFamily:'inherit',color:'#eef1f5',lineHeight:1.72,fontSize:15,margin:'14px 0 0',overflowWrap:'anywhere' }}>{result.email}</pre><div className="ta-outreach-actions"><button onClick={openMail} disabled={!canSend} style={{ border:0,background:canSend?'#e1ddd2':'#3a3f48',color:'#17191d',borderRadius:11,padding:'12px 15px',fontWeight:900,cursor:canSend?'pointer':'not-allowed' }}>{mailOpened?'Mail app opened':'Open in Mail'}</button><button onClick={()=>copy(result.email,'body2')} style={{ background:'#0d1118',color:'#e8ebf0',border:'1px solid #303948',borderRadius:11,padding:'12px 15px',fontWeight:800,cursor:'pointer' }}>Copy & Paste</button></div></section>
