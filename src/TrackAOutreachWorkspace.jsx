  ...
  return <div style={{ minHeight: '100vh', background: '#0b0d12', color: '#f3f4f7', fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', padding: '34px 24px 80px' }}>
    <div style={{ maxWidth: 1180, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 18, alignItems: 'flex-start', marginBottom: 24 }}>
        <div><div style={{ color: '#8d99ab', fontSize: 10, textTransform: 'uppercase', letterSpacing: '.18em', fontWeight: 900 }}>Cornerstone · Track A Revenue Recovery</div><h1 style={{ margin: '9px 0 8px', fontSize: 44, lineHeight: .98, letterSpacing: '-.05em' }}>Recovery conversation for {business.name || 'this business'}</h1><p style={{ margin: 0, color: '#98a2b1', maxWidth: 800, lineHeight: 1.65 }}>Qwen researches the revenue-leakage angle and writes a prospect-first message. The aim is to identify money already entering the business but disappearing before the deal closes.</p></div><div style={pill}>{status}</div>
      </div>
      ...
      {!result ? <section style={{ minHeight:330, display:'grid',placeItems:'center',...card }}><div style={{ textAlign:'center' }}><div style={{ fontSize:34, marginBottom:10 }}>↗</div><div style={{ fontSize:18,fontWeight:900 }}>Building the recovery outreach package</div><div style={{ color:'#7f899a',marginTop:7 }}>{status}</div>{jobId && <div style={{ color:'#586273',fontSize:10,marginTop:10 }}>Job {jobId.slice(0,8)}</div>}</div></section> : <div style={{ display:'grid',gap:14 }}>
        <section style={{ ...card, borderColor:'rgba(146,157,174,.28)' }}><div style={{ ...label }}>Recommended subject</div><div style={{ marginTop:8,fontSize:26,fontWeight:900,letterSpacing:'-.03em' }}>{result.recommended_subject}</div><div style={{ display:'flex',gap:8,flexWrap:'wrap',marginTop:12 }}>{(result.subject_options||[]).map((s,i)=><button key={`${s}-${i}`} onClick={()=>copy(s,`s${i}`)} style={{ background:'#0d1118',color:'#cfd5df',border:'1px solid #2a3340',borderRadius:10,padding:'8px 10px',cursor:'pointer' }}>{copied===`s${i}`?'Copied':`Option ${i+1}: ${s}`}</button>)}</div></section>
        <section style={card}><div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',gap:12 }}><div style={{ ...label }}>Recovery email</div><button onClick={()=>copy(result.email,'body')} style={{ background:'#0d1118',color:'#cfd5df',border:'1px solid #2a3340',borderRadius:10,padding:'7px 10px',cursor:'pointer' }}>{copied==='body'?'Copied':'Copy email'}</button></div><pre style={{ whiteSpace:'pre-wrap',fontFamily:'inherit',color:'#eef1f5',lineHeight:1.72,fontSize:15,margin:'14px 0 0' }}>{result.email}</pre><div style={{ marginTop:18,display:'flex',gap:9,flexWrap:'wrap' }}><button onClick={openMail} disabled={!canSend} style={{ border:0,background:canSend?'#e1ddd2':'#3a3f48',color:'#17191d',borderRadius:11,padding:'11px 15px',fontWeight:900,cursor:canSend?'pointer':'not-allowed' }}>{mailOpened?'Mail app opened':'Open in Mail'}</button><button onClick={()=>copy(result.email,'body2')} style={{ background:'#0d1118',color:'#e8ebf0',border:'1px solid #303948',borderRadius:11,padding:'11px 15px',fontWeight:800,cursor:'pointer' }}>Copy & Paste</button></div></section>
