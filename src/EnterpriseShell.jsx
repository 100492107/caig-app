  ...
  return (
    <div className="enterprise-shell">
      <style>{`
        ...
        .es-brand strong{display:block;font-size:12px;letter-spacing:.04em}.es-brand small{display:block;margin-top:2px;font-size:8px;letter-spacing:.16em;text-transform:uppercase;color:var(--text-subtle)}
        .es-nav{display:flex;gap:4px;align-items:center;flex:1;overflow:auto;scrollbar-width:none}.es-nav::-webkit-scrollbar{display:none}
        .es-nav a{position:relative;padding:9px 11px;border-radius:9px;color:var(--text-muted);font-size:10px;font-weight:820;text-decoration:none;white-space:nowrap}.es-nav a:hover{background:var(--surface);color:var(--text)}.es-nav a.active{background:var(--surface-2);color:var(--text)}.es-nav a.active::after{content:"";position:absolute;left:11px;right:11px;bottom:3px;height:1px;background:var(--track-b);opacity:.85}
        ...
        .es-subnav{display:flex;gap:6px;overflow:auto;margin:0 0 18px;padding-bottom:3px;scrollbar-width:none}.es-subnav::-webkit-scrollbar{display:none}.es-subnav a{color:var(--text-muted);text-decoration:none;font-size:10px;font-weight:800;padding:8px 10px;border:1px solid var(--border);border-radius:9px;white-space:nowrap}.es-subnav a.active{color:var(--text);border-color:rgba(212,181,106,.35);background:rgba(212,181,106,.08)}
      ...
      <header className="es-top">
        ...
        <nav className="es-nav" aria-label="Cornerstone">
          {NAV.map((item) => (
            <a key={item.id} href={item.href} className={active === item.id ? 'active' : ''} target={item.external ? '_blank' : undefined} rel={item.external ? 'noreferrer' : undefined}>{item.label}{item.external ? ' ↗' : ''}</a>
          ))}
        </nav>
