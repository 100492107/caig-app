  ...
  return (
    <div className="cs-app">
      <aside className="cs-rail" aria-label="Primary">
        <a className="cs-brand" href="/"><span className="cs-brand-mark">C</span><span className="cs-brand-text"><strong>Cornerstone</strong><span>Creator OS</span></span></a>
        <nav className="cs-rail-nav" aria-label="Workspace">
          {NAV.map(item=><a key={item.id} href={item.href} className={`cs-rail-link${resolved===item.id?' is-active':''}`} aria-current={resolved===item.id?'page':undefined}><span>{item.label}</span><kbd>{item.key}</kbd></a>)}
        </nav>
        <div className="cs-rail-foot">
          <div style={{padding:'0 12px 9px',fontSize:9,letterSpacing:'.13em',textTransform:'uppercase',color:'var(--cs-os-subtle)',fontWeight:800}}>Utilities</div>
      ...
      <div className="cs-stage">
        <div className="cs-stage-bar"><span className="label">{eyebrow||'Creator OS'}</span><span className="meta">Evidence → Build → Make → Publish → Learn</span></div>
