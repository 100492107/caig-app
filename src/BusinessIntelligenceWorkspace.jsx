 ...
 return <EnterpriseShell active="business" eyebrow="Business Intelligence">
   <main className="bi">
    ...
    {tab==='audience'&&<section>
      ...
      <div className="bi-grid2"><article className="bi-panel"><div className="bi-panel-head"><b>Owned creators</b><span>Latest recorded creator metrics</span></div>{creatorsM.length?creatorsM.map(c=><div className="bi-listrow" key={c.id||c.creator_id}><div><b>{({cara:'Cara Whitmore',lila:'Lila Sterling',cara_lila:'Cara + Lila',duo:'Cara + Lila'})[c.creator_id]||c.creator_id||'Creator'}</b><span>{c.platform||'Platform not set'}</span></div><strong>{num(c.audience_followers)} followers</strong></div>):<div className="bi-empty">No creator metric snapshots recorded.</div>}</article><article className="bi-panel"><div className="bi-panel-head"><b>Subscriber metrics</b><span>Platform snapshots only</span></div><div className="bi-big">{num(totalSubs)}</div><p className="bi-muted">{num(paidSubs)} paid subscribers across the latest recorded platform snapshots. Private subscriber memory is not queried from the browser.</p></article></div>
    </section>}
