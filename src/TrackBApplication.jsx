...
const GROUPS = [
    ...
    items: [
      {
        ...
        icon: "✦",
        stages: ["Discover", "Analyse", "Build", "Multiply", "Publish", "Monetise", "Measure"],
      },
    ],
  },
  {
    ...
    items: [
      ...
      { id: "publish", label: "Publish", icon: "↑", stages: ["Queue", "Schedule", "Status"] },
  ...
  },
  {
    label: "Tools",
    items: [
      { id: "media", label: "Media library", icon: "▦", stages: ["Library", "Sources", "Derived"] },
      ...
      { id: "shop", label: "Commerce tests", icon: "◇", stages: ["Offer", "Creative", "Test"] },
...
function Workspace({ id, stage, onAdvance }) {
  switch (id) {
    case "content-engine":
      return <ContentEngineWorkspace stage={stage} />;
    case "measurement":
    ...
    case "media":
      return <MediaStaged stage={stage} />;
    case "caption-writer":
...
export default function TrackBApplication() {
  const [view, setView] = useState(() => {
    try {
      return sessionStorage.getItem("caig_track_b_view") || "content-engine";
    } catch {
  ...
  const [collapsed, setCollapsed] = useState(() => {
    try {
      return localStorage.getItem("caig_tb_sidebar") === "1";
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem("caig_tb_sidebar", collapsed ? "1" : "0");
    } catch {}
  }, [collapsed]);
  ...
  const onAdvance = () => changeStage(stage + 1);
  ...
  return (
    <div className={`tb-app${collapsed ? " is-collapsed" : ""}`}>
      ...
      <aside className={`tb-sidebar${mobileOpen ? " is-open" : ""}`}>
        <div className="tb-brand">
          <a href="/" className="tb-brand-link" aria-label="Back to Cornerstone">
            ...
            <span className="tb-brand-copy">
              ...
              <span>Track B · primary</span>
            </span>
          </a>
          <button className="tb-mobile-close" type="button" onClick={() => setMobileOpen(false)} aria-label="Close navigation">
            ×
          </button>
          <button
        ...
        <nav className="tb-nav" aria-label="Content Engine navigation">
          {GROUPS.map((group) => (
      ...
      <main className="tb-main">
        <header className="tb-topbar">
          <div className="tb-topbar-inner">
            ...
            <LocalAIStatus compact />
            <div className="tb-stagebar">
              {stages.length > 0 && (
                <div className="tb-stage-nav" aria-label={`${current.label} stages`}>
                  {stages.map((s, i) => (
                    <button
                      key={s}
                      type="button"
                      ...
                      aria-label={`${s}, stage ${i + 1} of ${stages.length}`}
                    >
        ...
        <div className="tb-content">
          <div className="tb-workspace-body">
            <Workspace id={view} stage={stage} onAdvance={onAdvance} />
          </div>
          {stages.length > 1 && (
            <div className="tb-mobile-stage-controls">
              <button className="tb-secondary" disabled={stage === 0} onClick={() => changeStage(stage - 1)}>
                Back
              </button>
              <span aria-live="polite">{stages[stage]}</span>
              <button className="tb-primary" disabled={stage === stages.length - 1} onClick={() => changeStage(stage + 1)}>
                Next
              </button>
            </div>
          )}
