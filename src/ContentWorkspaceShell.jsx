...
function Workspace({stage}){switch(stage){case'remake':return <ContentEngineWorkspace onGo={(id)=>{const next=['production','publish','measurement','creators','profiles'].includes(id)?id:'production';window.location.href=`/content/${next}`}}/>;case'creators':return <CreatorsStaged stage={0} onAdvance={()=>{window.location.href='/content/profiles'}}/>;case'profiles':return <ProfileChannelsWorkspace/>;case'production':return <CanonicalProductionWorkspace/>;case'publish':return <CanonicalPublishWorkspace/>;case'measurement':return <CanonicalMeasureWorkspace/>;default:return null}}
