...
function money(usd) {
  ...
  return `$${n.toFixed(2)} (~£${(n * GBP_PER_USD).toFixed(2)})`;
}

function estimate(mode, resolution, outputs) {
  if (mode === "static_image" || mode === "carousel") return 0;
  const perClip = resolution === "480p" ? 0.2 : 0.4;
  return +(perClip * Math.max(1, Number(outputs) || 1)).toFixed(2);
}
...
export default function CornerstoneAIAssetsProductionStudioV4() {
  ...
  async function generateLive() {
    if (!selectedReferences[0]?.public_url) {
      setMessage("Select at least one approved image reference first.");
      return;
    }
    if (!["cinematic_motion", "ugc"].includes(mode)) {
      setMessage("The first live adapter currently supports Cinematic Motion and UGC. The other modes are planned shot-by-shot next.");
      return;
    }

    setBusy(true);
    setVideoUrl("");
    setMessage("Submitting the approved reference image to fal.ai Wan 2.1…");
    let projectId = null;
    let jobId = null;
    ...
    try {
      ...
      const { data: job, error: jobError } = await supabase
        ...
        .insert({
          ...
          config: {
            ...
            estimated_gbp: +(estimatedUsd * GBP_PER_USD).toFixed(2),
          },
          status: "processing",
        })
        .select("id")
      ...
      const submitResponse = await fetch("/api/track-b-video-submit", {
        ...
        body: JSON.stringify({
          imageUrl: selectedReferences[0].public_url,
          prompt,
          resolution,
          aspectRatio: "9:16",
          numFrames: 81,
      ...
      if (!submitResponse.ok) throw new Error(submit?.error || "Video submit failed");

      let completed = false;
      for (let attempt = 0; attempt < 80; attempt += 1) {
        await new Promise((resolve) => setTimeout(resolve, 4500));
        const statusResponse = await fetch("/api/track-b-video-poll", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ requestId: submit.requestId, action: "status" }),
        });
        const status = await statusResponse.json();
        if (!statusResponse.ok) throw new Error(status?.error || "Video status check failed");
        setMessage(`Generating… ${status.status}${status.queuePosition != null ? ` · queue ${status.queuePosition}` : ""}`);
      ...
      const resultResponse = await fetch("/api/track-b-video-poll", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ requestId: submit.requestId, action: "result" }),
      });
      ...
      setMessage(`Done — 1 live premium clip generated for ${money(estimatedUsd)}.`);
      await load();
    } catch (error) {
      if (jobId) {
        await supabase.from("track_b_production_jobs").update({
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#08070d", color: "#eef1f7", padding: "28px 32px 72px", fontFamily: "Inter,system-ui,sans-serif" }}>
      <div style={{ maxWidth: 1240, margin: "0 auto 18px" }}>
        ...
        <p style={{ margin: 0, color: "#7f8798", fontSize: 12 }}>Live first adapter: approved Asset Library image → fal.ai Wan 2.1 image-to-video.</p>
      </div>
      ...
      <div style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "1.15fr .85fr", gap: 16 }}>
        <section style={card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 10 }}>
            <label style={{ fontSize: 11, color: "#9ca5b6" }}>Character
              <select style={input} value={characterId} onChange={(e) => setCharacterId(e.target.value)}>
                <option value="">None</option>
                {characters.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </label>
            <label style={{ fontSize: 11, color: "#9ca5b6" }}>Brand
              <select style={input} value={brandId} onChange={(e) => { setBrandId(e.target.value); setProductId(""); }}>
                <option value="">None</option>
          ...
          <label style={{ display: "block", fontSize: 11, color: "#9ca5b6", marginTop: 10 }}>Product
          ...
          <div style={{ color: "#7f8798", fontSize: 11, margin: "5px 0 10px" }}>The first selected image is the actual source frame sent to the video model.</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 9 }}>
            {assets.filter((asset) => ["image", "reference"].includes(asset.asset_type)).map((asset) => {
              ...
              return (
                <button key={asset.id} onClick={() => toggleReference(asset.id)} style={{ padding: 0, textAlign: "left", overflow: "hidden", borderRadius: 12, border: `2px solid ${selected ? "#d4af37" : "#252a39"}`, background: "#0a0c12" }}>
                  {asset.public_url
                    ? <img src={asset.public_url} alt={asset.name} style={{ width: "100%", aspectRatio: "4/3", objectFit: "cover", display: "block" }} />
                    : <div style={{ aspectRatio: "4/3", display: "grid", placeItems: "center", color: "#667087" }}>NO URL</div>}
        ...
        <section style={card}>
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 10 }}>
            {MODES.map(([id, label, description]) => (
              <button key={id} onClick={() => setMode(id)} style={{ ...button, textAlign: "left", borderColor: mode === id ? "#d4af37" : "#303648", background: mode === id ? "rgba(212,175,55,.11)" : "#151924" }}>
                <div>{label}</div>
                <div style={{ color: "#7f8798", fontSize: 10, marginTop: 3 }}>{description}</div>
              </button>
            ))}
          </div>

          <label style={{ display: "block", fontSize: 11, color: "#9ca5b6", marginTop: 10 }}>Resolution
            <select style={input} value={resolution} onChange={(e) => setResolution(e.target.value)}>
              <option value="720p">720p — recommended</option>
              <option value="480p">480p — cheaper</option>
            </select>
          </label>

          <label style={{ display: "block", fontSize: 11, color: "#9ca5b6", marginTop: 10 }}>Outputs
            <select style={input} value={outputs} onChange={(e) => setOutputs(Number(e.target.value))}>
              {[1, 2, 3, 5].map((count) => <option key={count} value={count}>{count}</option>)}
            </select>
          </label>

          <label style={{ display: "block", fontSize: 11, color: "#9ca5b6", marginTop: 10 }}>Motion direction
            <textarea rows={6} style={{ ...input, resize: "vertical" }} value={motion} onChange={(e) => setMotion(e.target.value)} />
          </label>
          ...
          <div style={{ ...card, background: "#0a0c12", marginTop: 12 }}>
            ...
            <div style={{ fontSize: 24, fontWeight: 900, marginTop: 5 }}>{money(estimatedUsd)}</div>
            <div style={{ color: "#7f8798", fontSize: 10, marginTop: 5 }}>720p = $0.40 per ~5s clip; 480p = $0.20.</div>
          </div>

          <button style={{ ...primary, width: "100%", marginTop: 12 }} onClick={generateLive} disabled={busy || referenceIds.length === 0}>
            {busy ? "Generating…" : `Generate live clip · ${money(estimatedUsd)}`}
          </button>

          {videoUrl && (
            <div style={{ marginTop: 14 }}>
              <video src={videoUrl} controls playsInline style={{ width: "100%", borderRadius: 12, background: "#000" }} />
              <a href={videoUrl} target="_blank" rel="noreferrer" style={{ display: "inline-block", marginTop: 8, color: "#d4af37", fontSize: 11 }}>Open generated video →</a>
            </div>
          )}
      ...
      <section style={{ maxWidth: 1240, margin: "16px auto 0", ...card }}>
        ...
        {jobs.slice(0, 10).map((job) => {
          const config = asJson(job.config);
          return (
            <div key={job.id} style={{ borderTop: "1px solid #252a39", padding: "10px 0", display: "flex", justifyContent: "space-between", gap: 10 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 800 }}>{job.track_b_content_projects?.title || job.mode}</div>
                <div style={{ color: "#7f8798", fontSize: 10, marginTop: 3 }}>{job.status} · {money(Number(config.estimated_usd || 0))}</div>
              </div>
