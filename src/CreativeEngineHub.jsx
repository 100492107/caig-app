...
export default function CreativeEngineHub() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0c10", color: "#f5f6f8", fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display','SF Pro Text',Inter,system-ui,sans-serif", display: "flex" }}>
      <style>{`.cs-sidebar{width:236px;flex:0 0 236px;min-height:100vh;position:sticky;top:0;border-right:1px solid rgba(255,255,255,.06);background:#090b0f;display:flex;flex-direction:column}.cs-main{min-width:0;flex:1}.cs-topbar{height:64px;position:sticky;top:0;z-index:20;border-bottom:1px solid rgba(255,255,255,.055);background:rgba(10,12,16,.82);backdrop-filter:blur(20px)}.cs-inner{max-width:1440px;margin:0 auto;width:100%}.cs-mobile{display:none}@media(max-width:900px){.cs-sidebar{position:fixed;left:0;top:0;bottom:0;z-index:60;transform:translateX(-100%);transition:transform .2s ease;box-shadow:25px 0 70px rgba(0,0,0,.4)}.cs-sidebar.open{transform:translateX(0)}.cs-mobile{display:flex}.cs-topbar{height:58px}.cs-content{padding-top:10px!important}.cs-inner{padding-left:18px!important;padding-right:18px!important}}`}</style>
      ...
      <aside className={`cs-sidebar ${mobileOpen ? "open" : ""}`}>
        ...
        <div style={{ padding: "10px", borderTop: "1px solid rgba(255,255,255,.045)" }}><a href="/" style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 11px", borderRadius: 11, color: "#697383", textDecoration: "none", fontSize: 12, fontWeight: 700 }}><span>←</span><span>Enterprise</span></a></div>
      </aside>
      ...
      <section className="cs-main">
        <header className="cs-topbar"><div className="cs-inner" style={{ height: "100%", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 18 }}><div className="cs-mobile" style={{ alignItems: "center", gap: 12 }}><button type="button" onClick={() => setMobileOpen(true)} style={{ border: 0, background: "transparent", color: "#fff", fontSize: 21 }}>☰</button></div><div style={{ display: "flex", alignItems: "center", gap: 9, minWidth: 0 }}><span style={{ color: "#5d6675", fontSize: 12 }}>Creative Station</span><span style={{ color: "#343a45" }}>/</span><span style={{ fontSize: 13, fontWeight: 850 }}>{current.label}</span></div><div style={{ display: "flex", alignItems: "center", gap: 8, color: "#667182", fontSize: 10, fontWeight: 700 }}><span style={{ width: 6, height: 6, borderRadius: 50, background: "#72cda7", boxShadow: "0 0 12px rgba(114,205,167,.45)" }} />Local AI</div></div></header>
        <main className="cs-content cs-inner" style={{ padding: "28px 30px 70px" }}>
          {view === "autopilot" && <div style={{ marginBottom: 22 }}><h1 style={{ margin: 0, fontSize: "clamp(30px,4vw,46px)", lineHeight: 1, letterSpacing: "-.05em", fontWeight: 850 }}>What should we make next?</h1></div>}
          <Workspace view={view} />
        </main>
      </section>
    </div>
