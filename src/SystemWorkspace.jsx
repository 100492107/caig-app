...
export default function SystemWorkspace() {
  ...
  const online = Boolean(state.lastSeen && Date.now() - new Date(state.lastSeen).getTime() < 90000);
  ...
  return (
    <EnterpriseShell active="system" eyebrow="System">
      <main className="system-workspace">
        ...
        `}</style>
        <div className="sys-kicker">System</div>
        <h1 className="sys-title">What can stop the engine.</h1>
        <p className="sys-sub">Local AI, queue, and errors. Fix constraints here, then go back to Home.</p>
        <div className="sys-grid">
          ...
          <div className="sys-card"><strong>{loading ? '…' : state.jobs}</strong><span>Active jobs</span><small>Queued or processing.</small></div>
          <div className="sys-card"><strong className={state.errors ? 'sys-bad' : 'sys-ok'}>{loading ? '…' : state.errors}</strong><span>Recent errors</span><small>Last 100 jobs sampled.</small></div>
          <div className="sys-card"><strong>{loading ? '…' : state.currentJob || 'Idle'}</strong><span>Current job</span><small>From the local worker heartbeat.</small></div>
        </div>
        <section className="sys-panel">
          ...
          <div className="sys-command"><a className="primary" href="/">Back to Home →</a><a href="/content/remake">Open Content →</a></div>
        </section>
      </main>
    </EnterpriseShell>
  );
