...
export default function LocalAIStudio() {
  ...
  const [jobType, setJobType] = useState("creative_director");
  const [systemPrompt, setSystemPrompt] = useState(DEFAULT_SYSTEM);
  const [prompt, setPrompt] = useState("Create 5 short-form hooks for Cara about discipline and money. Make them specific, slightly blunt and British. No generic motivational quotes.");
  ...
  async function queue() {
    ...
    const { error } = await supabase.from("local_ai_jobs").insert({
      ...
      options: { max_tokens: 768, temperature: 0.8 },
      status: "queued",
    });
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", display: "grid", gridTemplateColumns: "1.1fr .9fr", gap: 16 }}>
        <section style={card}>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            ...
            <label style={{ fontSize: 11, color: "#9ca5b6" }}>Job type<select value={jobType} onChange={(e) => setJobType(e.target.value)} style={{ ...input, marginTop: 6 }}><option value="creative_director">Creative director</option><option value="caption_direction">Caption direction</option><option value="repurpose">Repurpose content</option><option value="fanvue_copy">Fanvue copy</option><option value="prompt_builder">Prompt builder</option></select></label>
            ...
            <label style={{ fontSize: 11, color: "#9ca5b6" }}>User prompt<textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={8} style={{ ...input, marginTop: 6, resize: "vertical" }} /></label>
        ...
        <section style={card}>
          ...
          <div style={{ color: "#7f8798", fontSize: 11, lineHeight: 1.6, marginBottom: 14 }}>The browser never talks directly to the model. The cloud queues a job in Supabase; the local worker claims it, calls the Qwen OpenAI-compatible server on localhost, then writes the result back.</div>
