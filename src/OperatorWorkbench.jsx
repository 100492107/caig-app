...
export default function OperatorWorkbench() {
  ...
  const [preflightTier, setPreflightTier] = useState("low");
  ...
  async function runSceneQa() {
    const contract = window.prompt("Paste the final scene contract JSON (location, timeOfDay, action, props, wardrobe):");
    if (!contract) return;
    let parsed; try { parsed = JSON.parse(contract); } catch { setMessage("Scene contract must be valid JSON."); return; }
    const checks = { location: Boolean(parsed.location), timeOfDay: Boolean(parsed.timeOfDay), action: Boolean(parsed.action), props: Array.isArray(parsed.props), wardrobe: Boolean(parsed.wardrobe) };
    const pass = Object.values(checks).every(Boolean);
    setMessage(pass ? "Scene contract structurally passes pre-flight. Visual verification should run on the final media before publish." : "Scene contract blocked: missing required scene fields.");
  }
