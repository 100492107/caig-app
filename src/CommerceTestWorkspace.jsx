...
export default function CommerceTestWorkspace() {
  ...
  async function generate() {
    ...
    try {
      ...
      setTests(generated);
      setMessage(`${generated.length} curated commerce concepts ready.`);
    } catch (error) {
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(320px,400px) minmax(0,1fr)", gap: 20, alignItems: "start" }}>
          <aside style={{ ...styles.panel, padding: 20, position: "sticky", top: 18 }}>
            ...
            <div style={{ display: "grid", gap: 15, marginTop: 16 }}>
              ...
              <button onClick={saveProduct} style={{ ...styles.button, width: "100%" }}>Save product</button>
              ...
              <label style={styles.label}>Batch<select value={count} onChange={(e) => setCount(e.target.value)} style={styles.input}><option value="1">1 concept</option><option value="5">5 concepts</option><option value="10">10 concepts</option></select></label>
