...
function parseResult(value) {
  ...
  for (let i = start; i < raw.length; i += 1) {
    ...
    if (c === open) depth += 1;
    else if (c === close && --depth === 0) { try { return JSON.parse(raw.slice(start, i + 1)); } catch { return null; } }
  }
  return null;
}

function readValue(object, keys) {
  for (const key of keys) { const value = object?.[key]; if (value !== undefined && value !== null && clean(value)) return value; }
  return null;
}
...
export default function GenerationsHumanViewer() {
  ...
  async function load() {
    ...
    try {
      ...
      let body = {}; try { body = text ? JSON.parse(text) : {}; } catch { throw new Error(text.slice(0, 180) || 'The saved work library returned an invalid response.'); }
  ...
  return <EnterpriseShell active="library">
    <div style={styles.page}>
      <header style={styles.header}><div><div style={styles.eyebrow}>LIBRARY</div><h1 style={styles.h1}>Your work, decisions and ideas.</h1><p style={styles.lede}>Everything Cornerstone has made for you, with the reasoning behind it. Nothing here needs developer knowledge.</p></div><button onClick={load} disabled={loading} style={styles.refresh}>{loading ? 'Updating' : 'Refresh'}</button></header>
...
const styles = {
  page:{fontFamily:'var(--sans)',color:'var(--text)',padding:'8px 0 70px'},
  ...
  h1:{margin:'10px 0 0',fontFamily:'var(--display)',fontSize:'clamp(42px,6.2vw,72px)',fontWeight:500,lineHeight:.96,letterSpacing:'-.055em',maxWidth:'13ch'},
  lede:{margin:'14px 0 0',maxWidth:'620px',fontSize:13,lineHeight:1.65,color:'var(--text-muted)'},
