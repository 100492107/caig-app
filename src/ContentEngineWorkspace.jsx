    ...
    <div className="ce-grid">
      <section className="ce-panel">
        ...
        <div className="ce-fields">
          ...
          <Input label="Reference URL" value={referenceUrl} onChange={setReferenceUrl} placeholder="Paste winning YouTube / TikTok / channel URL" />
        ...
        <div className="ce-actions"><button className="ce-btn primary" disabled={busy} onClick={stageAction}>{busy ? "Working…" : `Run ${STAGES[activeStage]} →`}</button><button className="ce-btn" disabled={busy || (!referenceUrl && !referenceNotes && !sourceFile)} onClick={() => run("ANALYSE REFERENCE")}>Analyse this reference</button><button className="ce-btn" disabled={busy} onClick={() => run("DISCOVER + RANK")}>Discover niche topics</button></div>
      ...
      <section className="ce-panel">
        ...
        {!result && <div className="ce-empty">Nothing here yet. Fastest path: paste a winning YouTube URL → press Analyse reference → review the package and Shorts on this side.</div>}
