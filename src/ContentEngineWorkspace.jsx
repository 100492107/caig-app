...
MISSION
Discover proven attention, understand why it works, build an original stronger asset, multiply it into derivatives, publish, measure and compound real winners into Creative DNA.

CORE LOOP
Discover -> Analyse -> Build -> Multiply -> Publish -> Monetise -> Measure -> Repeat.

REFERENCE RULE
Reference content is research about demand and mechanism, never a template. Never copy exact wording, narration, footage, music, creator identity, branding, distinctive thumbnail artwork or near-identical scene execution. The final work must be materially original.

SOURCE QUALITY
Prefer repeated public evidence over isolated viral outliers. Separate public-web evidence from owned analytics. Never invent views, retention, revenue, comments or audience reactions.

LONG-FORM STANDARD
Return a selected opportunity, stronger angle, 10 ranked titles, 3 thumbnail concepts, opening hook, complete spoken script, chapters, visual timeline, production notes, SEO/upload package and 5 follow-up ideas.

SHORTS STANDARD
Create standalone derivatives with source windows, hooks, titles/captions, platform notes and a reason each clip should work. Never manufacture weak clips just to hit a quantity.

MEASUREMENT
Return a publish sequence, baseline metrics to capture, test dimensions, winner rule and what to change when a package underperforms.

MONETISATION
Suggest measurable tests such as YouTube advertising where eligible, affiliates, TikTok Shop where available, Fanvue for appropriate owned creator assets, sponsorships, subscriptions, products and licensing. Never promise revenue.

QUALITY GATE
Reject weak ideas. Score evidence, audience fit, originality, packaging, retention, visual potential, production cost and commercial potential. Use USE / ADAPT / IGNORE for reference mechanisms.
    ...
    <div className="ce-grid">
      <section className="ce-panel">
        ...
        <div className="ce-fields">
          ...
          <Input label="Target length (minutes)" value={duration} onChange={setDuration} placeholder="20" type="number" />
        ...
        <div className="ce-file"><input type="file" accept="video/*,audio/*,.txt,.md,.srt,.vtt" onChange={(e) => { setSourceFile(e.target.files?.[0] || null); setSourcePath(""); setIngestionJobId(""); setSourceEvidence(null); setProgress(-1); }} />{sourceFile ? <div style={{marginTop:7}}><strong>{sourceFile.name}</strong> · {Math.round(sourceFile.size / 1024 / 1024 * 10) / 10} MB · private source-media pipeline</div> : <div style={{marginTop:7}}>Video/audio/transcript upload. Large sources use the private Supabase storage bucket and local FFmpeg + Whisper + Qwen stack.</div>}</div>
        {sourceEvidence && <div className="ce-source-card"><div className="ce-source-dot"/><div><div className="ce-mono">{sourceEvidence.ingestion?.metadata?.fileName || sourceFile?.name}</div><div style={{fontSize:9,color:"#969ca5",marginTop:3}}>frames {sourceEvidence.ingestion?.metadata?.frameCount ?? 0} · transcript {Math.round((sourceEvidence.ingestion?.transcript?.text || "").length / 1000)}k chars · Qwen source analysis complete</div></div><span className="ce-success">READY</span></div>}
        ...
        {savedQueueId && <div className="ce-footer">Saved production item: <span className="ce-mono">{savedQueueId}</span> · <a className="ce-anchor" href="/creative" onClick={() => {}}>Production Studio</a></div>}
        {message && !busy && <div className="ce-footer">{message}</div>}
      </section>

      <section className="ce-panel">
        <h2>Operating principle</h2>
        <p><strong style={{color:"#e4e2db"}}>Don't start with content.</strong> Start with evidence that people already choose, then rebuild the mechanism into an owned asset.</p>
        <div className="ce-meta-grid"><div className="ce-meta"><span>Source</span><b>{sourceFile ? "Media" : "Web / notes"}</b></div><div className="ce-meta"><span>Intelligence</span><b>Qwen</b></div><div className="ce-meta"><span>Vision</span><b>{sourceEvidence ? "Analysed" : "Ready"}</b></div><div className="ce-meta"><span>Storage</span><b>Private</b></div></div>
        <div className="ce-footer">Private source files are scoped to the authenticated operator. The worker retrieves them using a time-limited signed URL, extracts representative frames with FFmpeg, sends the media to local Whisper and Qwen Vision, then hands unified evidence to Qwen text before the Content Engine builds the final package.</div>
        <div className="ce-actions-bottom" style={{marginTop:12}}><a className="ce-anchor" href="/">Enterprise command centre</a><a className="ce-anchor" href="/creative">Track B workspaces</a></div>
      </section>
    </div>

    {!result ? <div className="ce-result"><section className="ce-output"><div className="ce-empty">Run the engine to create an evidence-led opportunity board and production package.</div></section></div> : <div className="ce-result">
      ...
      {sourceEvidence && <OutputBlock title="Unified source evidence"><div className="ce-meta-grid"><div className="ce-meta"><span>Duration</span><b>{Math.round(sourceEvidence.ingestion?.metadata?.duration || 0)}s</b></div><div className="ce-meta"><span>Frames</span><b>{sourceEvidence.ingestion?.metadata?.frameCount || 0}</b></div><div className="ce-meta"><span>Transcript</span><b>{Math.round((sourceEvidence.ingestion?.transcript?.text || "").split(/\s+/).filter(Boolean).length)} words</b></div><div className="ce-meta"><span>Confidence</span><b>{sourceEvidence.text_analysis?.confidence || "—"}</b></div></div><p style={{marginTop:10}}><strong style={{color:"#e2e3de"}}>Audience promise:</strong> {sourceEvidence.text_analysis?.audience_promise || "Not available."}</p><p><strong style={{color:"#e2e3de"}}>Original reconstruction:</strong> {sourceEvidence.text_analysis?.original_reconstruction || "Not available."}</p></OutputBlock>}
      {selected && <>
        ...
        <OutputBlock title="Visual timeline"><ul className="ce-list">{(selected.visual_timeline||[]).slice(0,100).map((shot,i)=><li key={i}><strong>{shot.start}-{shot.end}s</strong> · {shot.purpose} · {shot.visual_direction}</li>)}</ul></OutputBlock>
      ...
      <div className="ce-publish"><OutputBlock title="Publish plan"><ul className="ce-list">{(result.publish_plan?.sequence||[]).map((x,i)=><li key={i}>{x}</li>)}</ul><p><strong style={{color:"#e2e3de"}}>Winner rule:</strong> {result.publish_plan?.winner_rule}</p></OutputBlock><OutputBlock title="Monetisation tests"><ul className="ce-list">{(result.monetisation_tests||[]).map((x,i)=><li key={i}><strong>{x.route}</strong> · {x.test} · {x.metric}</li>)}</ul></OutputBlock></div>
      <OutputBlock title="Production handoff"><div className="ce-actions-bottom"><button className="ce-btn primary" disabled={busy} onClick={saveToProduction}>{savedQueueId ? "Saved to Production" : "Save approved package to Production"}</button>{savedQueueId && <a className="ce-anchor" href="/creative">Open Track B / Production Studio</a>}</div><div className="ce-note">Saving creates a persistent Track B content-queue item using the approved script, hook, visual timeline, thumbnail direction, publication plan and source evidence. The existing Production Studio can then render it through the local/MPT/premium provider strategy.</div></OutputBlock>
    </div>}
  </div>;
}
